...
CREATE TABLE m_contributors (
  ...
  SAME_NAME_EXIST_FLG TINYINT(1)        NOT NULL DEFAULT 0 COMMENT '同名者存在フラグ',
  DEL_FLG  TINYINT(1)      NOT NULL DEFAULT 0,
  INS_PG   VARCHAR(64)    NOT NULL,
  INS_ID   VARCHAR(64)    NOT NULL,
  INS_DT   DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
...
CREATE TABLE e_records (
  ...
  PAGE_RANGE       VARCHAR(1024) NULL                   COMMENT '収録ページ (pp.i-i, 12-13 等)',
...
CREATE TABLE e_record_urls (
  ...
  ONSTRAINT fk_eru_user FOREIGN KEY (USER_ID) REFERENCES m_users (ID),
